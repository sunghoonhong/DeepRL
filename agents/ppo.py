...
class PPO(Agent):
    def __init__(self, **kwargs):
        ...
        self.preprocess_obs = (lambda a, b=None, c=None, d=None: a)
    ...
    def train(self):
        ...
        for _ in range(self.epoch):
            ...
            for i in range(batch_num):
                ...
                self.critic.opt.apply_gradients(zip(grads, self.critic.trainable_variables))
                ...
                self.actor.opt.apply_gradients(zip(grads, self.actor.trainable_variables))
    ...
    def play(self, render=False, verbose=False, delay=0, ep_label=0):
        ...
        obs = self.env.reset()
        state = self.preprocess_obs(obs, self.resize, self.seqlen)
        if render:
            self.env.render()
        ...
        while not done:
            ...
            next_state = self.preprocess_obs(obs, self.resize, self.seqlen, state)
...
class ImageDiscretePPO(PPO):
    def __init__(self, **kwargs):
        ...
        self.critic = ac.ImageContinuousActor(self.critic_units)
